...
export default function CreateListing() {
  ...
  console.log(fileUrl);
  console.log(nftAbi);
  const projectId = '05b70c0153bc4696be113bcb847a154f'
  const uploadToIpfs =async () => {
    // Get any data that you want to upload
    

    // And upload the data with the upload function
    const uploadUrl = await upload({ data:[ipfsFile],
    options:
    {
      uploadWithGatewayUrl:true,
      uploadWitjoutDirectory:true
    }
    });
    console.log("Uploaded Ipfs",uploadUrl);
  ...
  const pageStyles = {
    ...
    backgroundSize: '100% 100%', // Cover the entire width and height
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await approveAndList(NftAddress,TokenId);
  ...
  return (
    <div  style={pageStyles}>
        {/* <FileUpload setUrl={setFileUrl} projectId={projectId}/>
      ...
      {ipfsUrl &&<div> Uploaded Url : {ipfsUrl}</div>}
